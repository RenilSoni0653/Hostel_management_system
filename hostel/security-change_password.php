...
$ai=$_SESSION['id'];
$udate=date('Y-m-d');
	$sql="SELECT password FROM security where password=?";
	...
	if($row_cnt>0)
	{
		$con="update security set password=?,updation_date=?  where id=?";
...
<?php $result ="SELECT password FROM security WHERE id=?";
